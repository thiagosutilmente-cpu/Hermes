    ...
    window.fetchTrendDiagnostic = async function() {
        ...
        try {
            ...
            if (fetchResult && fetchResult.reply) {
                ...
                let replyClean = fetchResult.reply.replace(/```json/g, '').replace(/```/g, '').trim();
                output.innerHTML = replyClean.replace(/\n/g, '<br>');
